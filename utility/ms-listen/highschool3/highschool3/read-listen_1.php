...
preg_match_all($pattern_select, $subject_title, $matches_select);
...
////        break;
////    } else {
    $result[$i]['A'] = $matches_select[1][$i * 3 + 0];
    $result[$i]['B'] = $matches_select[1][$i * 3 + 1];
    $result[$i]['C'] = $matches_select[1][$i * 3 + 2];
//
//
...
//    $result[$i]['no'] = str_pad($i + 1, 3, "0", STR_PAD_LEFT);
    ...
    $result[$i]['title'] = '<p>No.' . $i . '{mp3}' . $result[$i]['no'] . 'mp3{/mp3}</p>';
//
//str_replace(array("\n", "\r"), '', $str);
