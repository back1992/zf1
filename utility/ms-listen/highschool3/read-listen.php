...
//    $result[$i]['no'] = str_pad($i + 1, 3, "0", STR_PAD_LEFT);
    ...
    if (!!$silence_arr[$i] && !!$silence_arr[$i + 1]) {
        $result[$i]['title'] .= '<audio id="mp3" controls="" autoplay="false">';
        $result[$i]['title'] .= '  <source src="images/mp3/cq1.mp3#t=' . $silence_arr[$i] . ',' . $silence_arr[$i + 1] . '" type="audio/mpeg">';
        $result[$i]['title'] .= '</audio>';
    }
//    $result[$i]['title'] = <<<'EOD'
//<p>title</p>
//<audio id="mp3" controls='' autoplay='false'>
//    <source src="images/mp3/cq1.mp3#t=$silence_arr[$i],$silence_arr[$i+1]" type='audio/mpeg'>
//</audio>
//EOD;
//
//str_replace(array("\n", "\r"), '', $str);
